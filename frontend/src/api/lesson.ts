...
export const lessonApi = {
  ...
  updateLesson: async (username: string, lessonData: UpdateLessonRequest) => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.message || `Failed to update lesson: ${response.statusText}`);
      }
  ...
  deleteLesson: async (username: string, lessonName: string) => {
    try {
      const url = buildApiUrl(`/user/lesson/${username}/delete-lesson?lessonName=${encodeURIComponent(lessonName)}`);
      const response = await fetch(url, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.message || `Failed to delete lesson: ${response.statusText}`);
      }

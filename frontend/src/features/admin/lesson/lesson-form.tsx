...
import {
  ...
  useTopicsByLanguage,
  useLevelsByLanguage,
} from '@/hooks/admin/use-lessons-admin'
...
export function LessonForm({ onSubmit, onCancel, isLoading }: LessonFormProps) {
  ...
  const { data: topicsData, isLoading: topicsLoading } =
    useTopicsByLanguage(selectedLanguageId)
  const { data: levelsData, isLoading: levelsLoading } =
    useLevelsByLanguage(selectedLanguageId)

  // Set default to English when languages load
  React.useEffect(() => {
    if (languagesData?.data && selectedLanguageId === 0) {
      ...
      if (englishLanguage) {
        setValue('languageId', englishLanguage.id)
      }
    }
  }, [languagesData, selectedLanguageId, setValue])

  // Reset topic and level when language changes
  React.useEffect(() => {
    ...
    <form onSubmit={handleSubmit(onFormSubmit)} className='space-y-4'>
      <div className='space-y-2'>
        ...
        <Input
          id='draftName'
          {...register('draftName')}
          placeholder='Enter a temporary name for the lesson'
        />
        {errors.draftName && (
